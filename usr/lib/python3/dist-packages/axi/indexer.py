...
class Progress:
    ...
    def begin(self, task):
        self.task = task
        print("%s..." % self.task, end='')
        sys.stdout.flush()
        self.halfway = True
    def progress(self, percent):
        print("\r%s... %d%%" % (self.task, percent), end='')
        sys.stdout.flush()
        self.halfway = True
    def end(self):
...
class Indexer(object):
    ...
    def gen_documents_deb822(self, fnames):
        ...
        for fname in fnames:
            infd = open(fname)
            # In Python 3, you cannot tell() on a file object while
            # iterating so you have to pop the hood to get at the
            # underlying buffer.  In Python 2, the file object doesn't have
            # this attribute, or this restriction.
            try:
                tellable = infd.buffer
    ...
    def compareCacheToDb(self, cache, db):
        ...
        for (idx, m) in enumerate(db.postlist("")):
            ...
            pkg = doc.get_data()
            if bytes is not str and isinstance(pkg, bytes):
                # python3-apt requires strings.
    ...
    def rebuild(self, pkgfiles=None):
        ...
        with open(axi.XAPIANINDEX + ".tmp", "w") as out:
            print("auto", os.path.abspath(dbdir), file=out)
    ...
    def writePrefixes(self, pathname=axi.XAPIANDBPREFIXES):
        ...
        with open(pathname+".tmp", "w") as out:
        ...
        """).lstrip(), file=out)
            ...
            for name, info in sorted(prefixes.items(), key=itemgetter(0)):
                print("%s\t%s\t%s\t# %s" % (
                    name,
                    info.get("qp", "-"),
                    info.get("type", "-"),
                    info.get("desc", "(description is missing)")), file=out)
    ...
    def writeValues(self, pathname=axi.XAPIANDBVALUES):
        ...
        """).lstrip(), file=out)
            ...
            for name, idx in sorted(self.values.items(), key=itemgetter(1)):
                desc = self.values_desc[name]
                print("%s\t%d\t# %s" % (name, idx, desc), file=out)
    ...
    def writeDoc(self, pathname=axi.XAPIANDBDOC):
        ...
        """).lstrip() % (axi.XAPIANDBPATH, axi.XAPIANDBVALUES), file=out)

            for d in docinfo:
                print(" * %s: %s" % (d['name'], d['shortDesc']), file=out)
        ...
        """), file=out)
            for d in docinfo:
                ...
                print('='*len(d['name']), file=out)
                print(textwrap.dedent(d['fullDoc']), file=out)
                print(file=out)
