...
cat /var/spool/asterisk/monitor/onsite-ibm-raw.txt | jq -r '.[]' | grep transcript |  tr '[:upper:]' '[:lower:]' | sed -e 's/"transcript"://g' | awk '{$1=$1};1' | tr -d "\"" > /var/spool/asterisk/monitor/onsite-ibm-clean.txt
